...
// ── Sample icons ──────────────────────────────────────────────────────────────
const SAMPLES = {
  bell: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M18 8a6 6 0 1 0-12 0c0 7-3 9-3 9h18s-3-2-3-9"/><path d="M10.5 20.5a1.7 1.7 0 0 0 3 0"/></svg>',
  heart: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M12 20s-7-4.5-7-9.5A4 4 0 0 1 12 7a4 4 0 0 1 7 3.5C19 15.5 12 20 12 20Z"/></svg>',
  ...
  star: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="m12 3 2.7 5.6 6.1.9-4.4 4.3 1 6.1L12 18.6 6.6 20l1-6.1L3.2 9.5l6.1-.9L12 3Z"/></svg>',
};
...
// ── CSS generation — single source of truth for preview AND export ────────────
function buildCss({ trigger, preset, duration, easing, intensity }) {
  ...
  if (trigger === "hover") {
    ...
    css = `${sel} { transition: transform ${d} ${easing}; }\n${sel}:hover { ${map[preset]} }`;
  ...
  // Wrap in reduced-motion guard so the exported CSS is production-ready
  // and the live preview also respects the user's OS setting.
...
async function buildLottie({ svg, color, trigger, preset, duration, intensity }) {
  ...
  return {
    v: "5.7.4", fr, ip: 0, op, w: SIZE, h: SIZE,
    nm: `${preset}-${trigger}`, ddd: 0,
    assets: [{ id: "icon_0", w: SIZE, h: SIZE, u: "", p: png, e: 1 }],
    layers: [{ ddd: 0, ind: 1, ty: 2, nm: "icon", refId: "icon_0", sr: 1, ks, ao: 0, ip: 0, op, st: 0, bm: 0 }],
  };
}
...
function frameTransform(trigger, preset, intensity, p) {
  ...
  if (trigger === "idle") {
    ...
    // hover: there-and-back loop for GIF preview
...
export default function Animate() {
  ...
  // When navigated from Library, route state carries the SVG and name.
  const incomingIcon = location.state?.svg  ?? null;
  const incomingName = location.state?.name ?? null;

  const [svg, setSvg] = useState(incomingIcon ?? SAMPLES.bell);
  ...
  // When navigating here with a new icon, load it and reset motion save state.
  useEffect(() => {
    if (incomingIcon) {
      setSvg(incomingIcon);
      setColorMode(hasHardcodedColors(incomingIcon) ? "original" : "brand");
    }
    setMotionSaved(false);
    setMotionError(null);
  }, [incomingIcon]);

  // Save current motion config back to the icon's row in Supabase.
  async function saveMotion() {
    if (!iconId || !isConfigured || !supabase) return;
    setMotionSaving(true);
    setMotionError(null);
    setMotionSaved(false);
    const motion = { trigger, preset, duration, easing, intensity, colorMode };
    ...
    if (error) {
      setMotionError(error.message ?? "Save failed.");
    } else {
      setMotionSaved(true);
    }
  }
  ...
  function handleUpload(file) {
    ...
    if (!isSvg) {
      setUploadInfo({ name: file.name, kb: kb.toFixed(1), error: true, warn: "Not an SVG. This module is SVG-first." });
      return;
    }
    ...
    reader.onload = () => {
      ...
      const warn =
        kb > 50 ? "Large file — optimize at svgomg.net before using." :
        kb > 15 ? "A little heavy — optimizing with SVGO is recommended." :
        null;
  ...
  function copyCss() {
    const usage =
      trigger === "click" ? "\n\n/* Usage: toggle .is-active on click, remove on animationend */" :
  ...
  async function downloadLottie() {
    ...
    try {
      ...
      downloadFile(JSON.stringify(json, null, 2), `icon_${preset}_${trigger}.json`, "application/json");
    } finally {
      setBuilding(false);
    }
  }
  ...
  async function exportGif() {
    ...
    try {
      ...
      const gif = new window.GIF({ workers: 2, quality: 8, width: W, height: H, workerScript: wurl, transparent: null });
      ...
      gif.on("finished", (blob) => {
        downloadFile(blob, `icon_${preset}_${trigger}.gif`);
        setGifState("idle");
      });
      gif.render();
    } catch {
  ...
  const sizedSvg = (s, px) =>
    s.replace(/<svg([^>]*)>/i, `<svg$1 width="${px}" height="${px}" style="display:block">`);
    ...
    <div className="h-full flex flex-col bg-stone-50 text-stone-900">
      ...
      <div className="flex-none border-b border-stone-200 bg-white px-6 py-3 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            {iconId && (
              <button
                ...
                className="text-xs text-stone-400 hover:text-stone-700 transition flex items-center gap-1"
              >
            ...
            <p className="text-sm font-semibold tracking-tight">
              {incomingName ?? "Animate"}
            </p>
          </div>
          <p className="text-xs text-stone-400 mt-0.5">
            {iconId ? "Editing from library — save motion to write it back" : "Hover, click & idle micro-interactions"}
          </p>
        </div>

        <div className="flex items-center gap-3">
          {/* Save motion — only visible when editing a library icon */}
          {iconId && (
            <div className="flex items-center gap-2">
              {motionError && (
                <span className="text-xs text-red-600">{motionError}</span>
              )}
              {motionSaved && !motionError && (
                <span className="text-xs text-stone-400">Motion saved</span>
              )}
              <button
                onClick={saveMotion}
                disabled={motionSaving || !isConfigured}
                title={!isConfigured ? "Supabase not configured" : undefined}
                className="flex items-center gap-1.5 text-xs font-medium px-3 py-1.5 rounded-lg bg-stone-900 text-white hover:bg-stone-800 transition disabled:opacity-40 disabled:cursor-not-allowed"
              >
                {motionSaving ? (
                  "Saving…"
                ) : motionSaved ? (
        ...
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-xs text-stone-400 ml-1" style={mono}>{color.toUpperCase()}</span>
        </div>
      </div>

      {/* Main grid */}
      <div className="flex-1 overflow-auto grid" style={{ gridTemplateColumns: "300px 1fr" }}>
        ...
        <div className="border-r border-stone-200 bg-white overflow-y-auto p-5 space-y-6">
          ...
          <div>
            ...
            <div className="flex gap-2 mb-2">
              {Object.entries(SAMPLES).map(([k, v]) => (
                <button
                  key={k}
                  onClick={() => { setSvg(v); setColorMode("brand"); setUploadInfo(null); }}
                  className={
                    "w-10 h-10 rounded-lg border flex items-center justify-center transition " +
                    (svg === v ? "border-stone-900 bg-stone-50" : "border-stone-200 hover:border-stone-400")
                  }
                  style={{ color }}
                  dangerouslySetInnerHTML={{ __html: sizedSvg(v, 22) }}
                />
              ))}
            </div>
            <textarea
            ...
            <button
              ...
              className="w-full mt-2 text-xs font-medium py-2 rounded-lg border border-stone-200 hover:bg-stone-50 transition flex items-center justify-center gap-1.5"
            >
            ...
            />
            <p className="text-xs text-stone-400 mt-1.5">SVG only · ideal under 15 KB</p>
            {uploadInfo && (
              <p className={"text-xs mt-1 " + (uploadInfo.error ? "text-red-600" : uploadInfo.warn ? "text-amber-700" : "text-stone-500")}>
                {uploadInfo.name} · {uploadInfo.kb} KB{uploadInfo.warn ? ` — ${uploadInfo.warn}` : uploadInfo.error ? "" : " ✓"}
              </p>
            )}
            ...
            <div className="mt-3">
              ...
              <Segmented
                options={[{ id: "brand", label: "Brand color" }, { id: "original", label: "Original" }]}
                value={colorMode}
                onChange={setColorMode}
              />
              <p className="text-xs text-stone-400 mt-1.5">
                {colorMode === "brand"
                  ? "Recolors icon paths to the brand color."
                  : "Keeps the icon's own colors."}
              </p>
            </div>
          </div>
          ...
          <div>
            <Label>Trigger</Label>
            <Segmented
              options={[{ id: "hover", label: "Hover" }, { id: "click", label: "Click" }, { id: "idle", label: "Idle" }]}
              value={trigger}
        ...
        <div className="p-6 overflow-y-auto">
          <div className="grid gap-6" style={{ gridTemplateColumns: "1fr 304px" }}>

            {/* Preview */}
            <div>
              <p className="text-xs font-medium text-stone-500 uppercase tracking-wide mb-3">
                Preview
                {trigger === "hover" && " — hover the icon"}
                {trigger === "click" && " — click the icon"}
              </p>

              <div
                className="bg-white border border-stone-200 rounded-xl flex items-center justify-center"
                style={{ height: 280 }}
              >
                <div
                  ...
                  dangerouslySetInnerHTML={{ __html: sizedSvg(renderSvg, 64) }}
                />
              </div>
              ...
              <div className="flex gap-3 mt-3">
                {[20, 32].map((px) => (
                  <div
                    key={px}
                    className="flex-1 bg-white border border-stone-200 rounded-lg py-3 flex items-center justify-center gap-2"
                    style={{ color }}
                  >
                    <div
                      className={"icon-anim" + (active ? " is-active" : "")}
                      dangerouslySetInnerHTML={{ __html: sizedSvg(renderSvg, px) }}
                    />
                    <span className="text-xs text-stone-400" style={mono}>{px}px</span>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="space-y-4">
              ...
              <div className="bg-white border border-stone-200 rounded-xl p-4">
                ...
                <pre
                  className="text-xs bg-stone-50 border border-stone-100 rounded-lg p-3 overflow-x-auto"
                  style={{ ...mono, maxHeight: 160 }}
                >
                  {css}
                </pre>
                <button
                  onClick={copyCss}
                  className="w-full mt-3 text-xs font-medium py-2 rounded-lg bg-stone-900 text-white hover:bg-stone-800 transition"
                >
                ...
                {lottieAvailable ? (
                  <>
                    <p className="text-xs text-stone-500 mb-3">
                      Icon embedded as an image layer with motion baked in.
                    </p>
                    <button
                      onClick={downloadLottie}
                      disabled={building}
                      className="w-full text-xs font-medium py-2 rounded-lg border border-stone-200 hover:bg-stone-50 transition disabled:opacity-50"
                    >
                  ...
                  <p className="text-xs text-stone-500">
                    Hover is CSS-native — use the CSS export above. Lottie is available for click and idle.
                  </p>
                )}
              </div>
              ...
              <div className="bg-white border border-stone-200 rounded-xl p-4">
                ...
                <div className="flex items-center gap-2 mb-3">
                  ...
                  />
                  <span className="text-xs text-stone-500" style={mono}>Background {gifBg.toUpperCase()}</span>
                </div>
                <button
                  onClick={exportGif}
                ...
                {gifState === "error" && (
                  <p className="text-xs text-amber-700 mt-2">Encoding failed — try once more.</p>
                )}
                <p className="text-xs text-stone-400 mt-2">
                  Use a background matching where the icon will sit — GIF transparency doesn't anti-alias well.
                </p>
              </div>
